...
#include <stdio.h>//add by hdzhang :for file operation
...
static __inline int skip_chunk (mpeg2dec_t * mpeg2dec, int bytes)
{
    uint8_t * current;
    uint32_t shift;
    uint8_t * limit;
...
static __inline int copy_chunk (mpeg2dec_t * mpeg2dec, int bytes)
{
    uint8_t * current;
    uint32_t shift;
    uint8_t * chunk_ptr;
...
static __inline mpeg2_state_t seek_chunk (mpeg2dec_t * mpeg2dec)
{
    int size, skipped;
...
mpeg2_state_t mpeg2_parse (mpeg2dec_t * mpeg2dec)
{
    ...
    switch (mpeg2dec->code) 
	{
		...
		case 0xb7:
			//所有帧解压完后执行该处，然后执行mpeg2_header_end。然后mpeg2_seek_header，应该没有header了，就退出
...
//by :因为该函数的引用被屏蔽，所以mpeg2dec->convert==0
...
mpeg2dec_t * mpeg2_init (void)
{
	...
	return NULL;
    #if 0 
    mpeg2dec->decoder.DCTblock =((int) (&(mpeg2dec -> decoder.DCTblock0[0]) + 16) )& (0xFFFFFF00);
    #endif 
    memset (mpeg2dec->decoder.DCTblock, 0, 64 * sizeof (int16_t));
    printf("addr :%0xd \n",&(mpeg2dec->decoder.DCTblock[0]));
